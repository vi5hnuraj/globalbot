...
const Loan = () => {
  ...
  const { data: userUSDCBalance, isLoading: loadingUSDCToken } = useBalance(import.meta.env.VITE_PAYMENT_MODE === 'BOT' ? undefined : usdcAddress);
  ...
  useEffect(() => {
    if (loadingUSDCToken || !userUSDCBalance) {
      ...
      setBalance(`${totalBalance} ${import.meta.env.VITE_PAYMENT_MODE === 'BOT' ? 'BOT' : 'USDC'}`);
    }
  ...
  const handleArena = async () => {
    try {
      const toastId = toast.loading(`Depositing 5 ${import.meta.env.VITE_PAYMENT_MODE === 'BOT' ? 'BOT' : 'USDC'} to lock Arena Smart Contract...`);
      setTimeout(() => {
  ...
  const handleFlash = async () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(async () => {
        ...
        const profitLabel = `+${profitValue} ${import.meta.env.VITE_PAYMENT_MODE === 'BOT' ? 'BOT' : 'USDC'} (${mockProfitPct}%)`;
    ...
    <>
      <div className="min-h-screen bg-black text-white p-8">
        ...
        <div className="bg-zinc-900 border-zinc-700 border-[1px] p-6 rounded-lg shadow-lg">
          <p className="text-lg mb-2">Required Balance: <span className="font-bold">5 {import.meta.env.VITE_PAYMENT_MODE === 'BOT' ? 'BOT' : 'USDC'}</span></p>
        ...
        <p className='text-2xl text-green-500' ><em>Deposit 5 {import.meta.env.VITE_PAYMENT_MODE === 'BOT' ? 'BOT' : 'USDC'} to lock the arena!</em></p>
        <br />
